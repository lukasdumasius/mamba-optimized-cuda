...
// WMMA API is only available on compute capability 7.0+
#include <mma.h>
#include <cuda_pipeline.h>
__host__ __device__ inline int div_up(int a, int b) { return (a + b - 1) / b; }

#ifndef BLOCK_M
#define BLOCK_M 16 // rows in chunk (tile of chunk_size)
#endif
#ifndef BLOCK_N
...
__global__ void chunk_scan_fwd_kernel(
    ...
    const int64_t*  __restrict__ seq_idx,   // for scale_m
    const scalar_t* __restrict__ C,         // (batch, seqlen, ngroups, dstate)
    const scalar_t* __restrict__ states,    // (batch, nchunks, nheads, headdim, dstate)
    const scalar_t* __restrict__ D,         // optional
    // outputs
    ...
    // strides
    int64_t stride_cb_batch, int64_t stride_cb_chunk, int64_t stride_cb_group,
    int64_t stride_cb_m,     int64_t stride_cb_k,
    int64_t stride_x_batch,  int64_t stride_x_seqlen,
    int64_t stride_x_head,   int64_t stride_x_hdim,
    int64_t stride_dt_batch, int64_t stride_dt_head,
    int64_t stride_dt_chunk, int64_t stride_dt_csize,
    int64_t stride_dA_batch, int64_t stride_dA_head,
    int64_t stride_dA_chunk, int64_t stride_dA_csize,
    int64_t stride_C_batch,  int64_t stride_C_seqlen,
    int64_t stride_C_group,  int64_t stride_C_dstate,
    int64_t stride_states_batch,  int64_t stride_states_chunk,
    int64_t stride_states_head,   int64_t stride_states_hdim,
    int64_t stride_states_dstate,
    int64_t stride_out_batch, int64_t stride_out_seqlen,
    int64_t stride_out_head, int64_t stride_out_hdim,
    ...
    // columns handled by this thread
    ...
    // base pointers
    const scalar_t* cb_base = cb
        + pid_b * stride_cb_batch
        + pid_c * stride_cb_chunk
        + group_idx * stride_cb_group;

    const scalar_t* x_base = x
        + pid_b * stride_x_batch
        + seq_start * stride_x_seqlen
        + pid_h * stride_x_head;

    const scalar_t* dt_base = dt
        + pid_b * stride_dt_batch
        + pid_h * stride_dt_head
        + pid_c * stride_dt_chunk;

    const float* dA_base = dA_cumsum
        + pid_b * stride_dA_batch
        + pid_h * stride_dA_head
        + pid_c * stride_dA_chunk;

    const scalar_t* C_base = C
        + pid_b * stride_C_batch
        + seq_start * stride_C_seqlen
        + group_idx * stride_C_group;

    const scalar_t* states_base = states
    ...
    scalar_t* out_base = out
        + pid_b * stride_out_batch
        + seq_start * stride_out_seqlen
    ...
    //  Precompute dA_m + scale_m (for C@states) 
    ...
    if (has_col0) {
        for (int i = 0; i < m_len; ++i) {
            ...
            if (!HAS_SEQ_IDX) {
                ...
                int64_t seq_m =
                    (seq_pos >= 0 && seq_pos < seqlen)
                        ? seq_idx_base[seq_pos]
                        : (int64_t)-1;
    ...
    //  Shared memory for WMMA C@states + cb tiles 
    extern __shared__ char smem_pool[];
    __half* smem_C = reinterpret_cast<__half*>(smem_pool); // [BLOCK_M][BLOCK_D+8]
    __half* smem_States = reinterpret_cast<__half*>(
        smem_pool + BLOCK_M * (BLOCK_D + 8) * sizeof(__half));
    float* smem_Accum = reinterpret_cast<float*>(
        smem_pool
        + BLOCK_M * (BLOCK_D + 8) * sizeof(__half)
        + BLOCK_D * (BLOCK_N + 8) * sizeof(__half));
    __half* smem_cb = reinterpret_cast<__half*>(
        smem_pool
        + BLOCK_M * (BLOCK_D + 8) * sizeof(__half)
        + BLOCK_D * (BLOCK_N + 8) * sizeof(__half)
        + BLOCK_M * (BLOCK_N + 8) * sizeof(float));
    ...
    for (int kd = 0; kd < dstate; kd += BLOCK_D) {
        ...
        for (int i = tid; i < BLOCK_M * BLOCK_D; i += blockDim.x) {
            ...
            if (global_m < chunk_size_lim && global_k < dstate) {
                const scalar_t* src = C_base
                    + global_m * stride_C_seqlen
                    + global_k * stride_C_dstate;
        ...
        // load states tile [BLOCK_D x BLOCK_N]
        for (int i = tid; i < BLOCK_D * BLOCK_N; i += blockDim.x) {
            ...
            if (global_k < dstate && global_n < headdim) {
                const scalar_t* src = states_base
                    + global_n * stride_states_hdim
                    + global_k * stride_states_dstate;
        ...
        for (int w_m = warpId * WMMA_M; w_m < BLOCK_M; w_m += num_warps * WMMA_M) {
            ...
            for (int k_step = 0; k_step < BLOCK_D; k_step += WMMA_K) {
                ...
                for (int ni = 0; ni < 4; ++ni) {
                    ...
                    nvcuda::wmma::mma_sync(acc_frags[ni],
                                           a_frag, b_frag, acc_frags[ni]);
                }
            }

            // store into smem_Accum
            #pragma unroll
            for (int ni = 0; ni < 4; ++ni) {
                int n_step = ni * WMMA_N;
                float* dest = smem_Accum + w_m * (BLOCK_N + 8) + n_step;
                nvcuda::wmma::store_matrix_sync(dest,
                                                acc_frags[ni],
                                                BLOCK_N + 8,
    ...
    if (has_col0) {
        ...
        for (int i = 0; i < m_len; ++i) {
            acc_t base0 = (local_n0 >= 0 && local_n0 < BLOCK_N)
                ? (acc_t)smem_Accum[i * (BLOCK_N + 8) + local_n0]
                : (acc_t)0;
            acc_col0[i] = base0 * scale_m_arr[i];

            if (has_col1) {
                acc_t base1 = (local_n1 >= 0 && local_n1 < BLOCK_N)
                    ? (acc_t)smem_Accum[i * (BLOCK_N + 8) + local_n1]
                    : (acc_t)0;
    ...
    __syncthreads(); // before reusing smem_cb

    int K_MAX = IS_CAUSAL ? min((pid_m + 1) * BLOCK_M, chunk_size_lim)
                          : chunk_size_lim;
        ...
        for (int k0 = 0; k0 < K_MAX; k0 += BLOCK_K) {
            ...
            if (lane < m_len) {
                ...
                for (int kk = 0; kk < klen; ++kk) {
                    int k_idx = k0 + kk;
                    if (k_idx < K_MAX) {
                        const scalar_t* cbptr = cb_base
                            + m_idx * stride_cb_m
                            + k_idx * stride_cb_k;
                        smem_cb[i * (BLOCK_K + 8) + kk] =
                            (__half)__ldg(cbptr);
                    } else {
            ...
            for (int kk = 0; kk < klen; ++kk) {
                ...
                const scalar_t* base_ptr0 = x_base
                    + k_idx * stride_x_seqlen
                    + n_idx0 * stride_x_hdim;
                ...
                if constexpr (USE_VEC2) {
                    if (has_col1 && stride_x_hdim == 1) {
                        ...
                        if (has_col1) {
                            const scalar_t* base_ptr1 = x_base
                                + k_idx * stride_x_seqlen
                                + n_idx1 * stride_x_hdim;
            ...
            for (int i = 0; i < m_len; ++i) {
                ...
                for (int kk = 0; kk < k_end; ++kk) {
                    ...
                    if constexpr (USE_BRANCHLESS) {
                        scale = fminf(expf_dA_m[i] * expf_neg_dA_k[kk], 1.0f);
                    } else {
    ...
    if constexpr (HAS_D) {
        for (int i = 0; i < m_len; ++i) {
            ...
            const scalar_t* xptr0 = x_base
                + m_idx * stride_x_seqlen
                + n_idx0 * stride_x_hdim;
    ...
    for (int i = 0; i < m_len; ++i) {
        ...
        if constexpr (HAS_Z) {
            // save out_x (pre-gating)
            out_x_base[m_idx * stride_out_seqlen + n_idx0 * stride_out_hdim] =
                (scalar_t)val0;

            if constexpr (USE_VEC2_Z) {
                if (has_col1) {
                    const float2* zptr2 = reinterpret_cast<const float2*>(
                        z_base + m_idx * stride_x_seqlen + n_idx0 * stride_z_hdim);
                    ...
                    out_x_base[m_idx * stride_out_seqlen + n_idx1 * stride_out_hdim] =
                        (scalar_t)val1;
                    float zf1 = (float)(scalar_t)zv.y;
                    float gated1 = zf1 * (1.0f / (1.0f + expf(-zf1)));
                    val1 *= (acc_t)gated1;
                } else {
                    // fall back to scalar path if we don't have col1
                    scalar_t z0 = __ldg(
                        z_base + m_idx * stride_x_seqlen + n_idx0 * stride_z_hdim);
                    float zf0 = (float)z0;
                    float gated0 = zf0 * (1.0f / (1.0f + expf(-zf0)));
                ...
                if (has_col1) {
                    out_x_base[m_idx * stride_out_seqlen + n_idx1 * stride_out_hdim] =
                        (scalar_t)val1;
                    scalar_t z1 = __ldg(
                        z_base + m_idx * stride_x_seqlen + n_idx1 * stride_z_hdim);
                    float zf1 = (float)z1;
                    float gated1 = zf1 * (1.0f / (1.0f + expf(-zf1)));
                    val1 *= (acc_t)gated1;
                }
        ...
        if constexpr (USE_VEC2_OUT) {
            if (has_col1) {
                float2* outptr2 = reinterpret_cast<float2*>(
                    out_base + m_idx * stride_out_seqlen + n_idx0 * stride_out_hdim);
                float2 outv;
                outv.x = (float)val0;
                outv.y = (float)val1;
                *outptr2 = outv;
            ...
            out_base[m_idx * stride_out_seqlen + n_idx0 * stride_out_hdim] =
                (scalar_t)val0;
            if (has_col1) {
                out_base[m_idx * stride_out_seqlen + n_idx1 * stride_out_hdim] =
                    (scalar_t)val1;
            }
        }
    }
...
inline void launch_chunk_scan_fwd_kernel(
    ...
    ><<<grid, block, shmem_bytes, stream>>>(
        cb_ptr, x_ptr, z_ptr, dt_ptr, dA_ptr, seq_ptr,
        C_ptr, states_ptr, D_ptr,
        out_ptr, out_x_ptr,
        batch, seqlen, nheads, headdim,
        nchunks, chunk_size, ngroups, dstate,
        stride_cb_batch, stride_cb_chunk, stride_cb_group,
        stride_cb_m, stride_cb_k,
        stride_x_batch, stride_x_seqlen,
        stride_x_head, stride_x_hdim,
        stride_dt_batch, stride_dt_head,
        stride_dt_chunk, stride_dt_csize,
        stride_dA_batch, stride_dA_head,
        stride_dA_chunk, stride_dA_csize,
        stride_C_batch, stride_C_seqlen,
        stride_C_group, stride_C_dstate,
        stride_states_batch, stride_states_chunk,
        stride_states_head, stride_states_hdim,
        stride_states_dstate,
        stride_out_batch, stride_out_seqlen,
        stride_out_head, stride_out_hdim,
        stride_z_hdim
    );
}
...
inline void dispatch_vec_variant(
    ...
    if (vec_all) {
        launch_chunk_scan_fwd_kernel<
            scalar_t,
            HAS_Z, HAS_D, D_HAS_HDIM, HAS_SEQ_IDX,
            /*USE_VEC2*/true,
            /*USE_VEC2_Z*/true,
            /*USE_VEC2_OUT*/true,
            USE_BRANCHLESS
...
std::vector<torch::Tensor> chunk_scan_fwd_cuda(
    ...
    int batch      = (int)cb.size(0);
    int nchunks    = (int)cb.size(1);
    int ngroups    = (int)cb.size(2);
    int chunk_size = (int)cb.size(3);
    int seqlen     = (int)x.size(1);
    int nheads     = (int)x.size(2);
    int headdim    = (int)x.size(3);
    int dstate     = (int)C.size(3);
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(x.scalar_type(), "chunk_scan_fwd_cuda", [&] {
        ...
        bool can_vec2     = kIsFloat && (stride_x_hdim == 1);
        bool can_vec2_z   = kIsFloat && HAS_Z && (stride_z_hdim == 1);
        bool can_vec2_out = kIsFloat && (stride_out_hdim == 1);
        bool vec_all = can_vec2 && can_vec2_z && can_vec2_out;

        // Adaptive branchless heuristic: use branchless for smaller workloads
        // Based on benchmark data: branchless is faster when batch * nchunks * nheads < 16
        bool use_branchless = (batch * nchunks * nheads) < 16;
        ...
        bool d_has_hdim = HAS_D && D.defined() && (D.dim() == 2);

        // Macro to dispatch with branchless parameter
        #define DISPATCH_BRANCHLESS(HAS_Z_V, HAS_D_V, D_HAS_HDIM_V, HAS_SEQ_IDX_V) \
            if (use_branchless) { \
                dispatch_vec_variant<scalar_t, HAS_Z_V, HAS_D_V, D_HAS_HDIM_V, HAS_SEQ_IDX_V, /*USE_BRANCHLESS*/true>( \
                    vec_all, grid, block, shmem_bytes, stream, \
                    cb_ptr, x_ptr, z_ptr, dt_ptr, dA_ptr, seq_ptr, \
                    C_ptr, states_ptr, D_ptr, out_ptr, out_x_ptr, \
                ...
                dispatch_vec_variant<scalar_t, HAS_Z_V, HAS_D_V, D_HAS_HDIM_V, HAS_SEQ_IDX_V, /*USE_BRANCHLESS*/false>( \
                    vec_all, grid, block, shmem_bytes, stream, \
                    cb_ptr, x_ptr, z_ptr, dt_ptr, dA_ptr, seq_ptr, \
                    C_ptr, states_ptr, D_ptr, out_ptr, out_x_ptr, \
                    batch, seqlen, nheads, headdim, nchunks, chunk_size, ngroups, dstate, \
        ...
        // template dispatch over flags
        if (HAS_Z) {
            if (HAS_D) {
                if (HAS_SEQ_IDX) {
                    if (d_has_hdim) {
                        DISPATCH_BRANCHLESS(true, true, true, true)
                    } else {
                        DISPATCH_BRANCHLESS(true, true, false, true)
                    }
                } else {
                    if (d_has_hdim) {
                        DISPATCH_BRANCHLESS(true, true, true, false)
                    } else {
                        DISPATCH_BRANCHLESS(true, true, false, false)
                    }
                }
            } else {
                if (HAS_SEQ_IDX) {
                    DISPATCH_BRANCHLESS(true, false, false, true)
                } else {
                    DISPATCH_BRANCHLESS(true, false, false, false)
                }
            }
        } else {
            if (HAS_D) {
                if (HAS_SEQ_IDX) {
                    if (d_has_hdim) {
                        DISPATCH_BRANCHLESS(false, true, true, true)
                    } else {
                        DISPATCH_BRANCHLESS(false, true, false, true)
                    }
                } else {
                    if (d_has_hdim) {
                        DISPATCH_BRANCHLESS(false, true, true, false)
                    } else {
                        DISPATCH_BRANCHLESS(false, true, false, false)
                    }
                }
            } else {
                if (HAS_SEQ_IDX) {
                    DISPATCH_BRANCHLESS(false, false, false, true)
                } else {
                    DISPATCH_BRANCHLESS(false, false, false, false)
                }
            }
        }

        #undef DISPATCH_BRANCHLESS
