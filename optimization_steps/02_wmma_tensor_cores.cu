    ...
    if (HAS_D) {
        for (int i = 0; i < m_len; ++i) {
            ...
            if (has_col1) {
                ...
                acc_t Dv1 = D_HAS_HDIM ? (acc_t)__ldg(D + pid_h * headdim + n_idx1) : (acc_t)__ldg(D + pid_h);
...
std::vector<torch::Tensor> chunk_scan_fwd_cuda(
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(x.scalar_type(), "chunk_scan_fwd_cuda", ([&] {
        ...
        size_t shmem_bytes = smem_C_bytes + smem_States_bytes + smem_Accum_bytes;

        constexpr bool kIsFloat = std::is_same<scalar_t, float>::value;
        int64_t stride_z_hdim = HAS_Z ? z.strides()[3] : 0;
        bool use_vec2 = kIsFloat && (x_strides[3] == 1);
        bool use_vec2_z = kIsFloat && HAS_Z && (stride_z_hdim == 1);
        bool use_vec2_out = kIsFloat && (out_strides[3] == 1);

        dim3 block(64);
        auto stream = at::cuda::getCurrentCUDAStream();

        chunk_scan_fwd_kernel<scalar_t, acc_t><<<grid, block, shmem_bytes, stream>>>(
            cb.data_ptr<scalar_t>(), x.data_ptr<scalar_t>(),
            HAS_Z ? z.data_ptr<scalar_t>() : nullptr,
            dt.data_ptr<scalar_t>(), dA_cumsum.data_ptr<float>(),
            HAS_SEQ_IDX ? seq_idx.data_ptr<int64_t>() : nullptr,
            C.data_ptr<scalar_t>(), states.data_ptr<scalar_t>(),
            HAS_D ? D.data_ptr<scalar_t>() : nullptr,
            out.data_ptr<scalar_t>(), HAS_Z ? out_x.data_ptr<scalar_t>() : nullptr,
